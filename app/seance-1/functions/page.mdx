...
**La syntaxe moderne en JavaScript !**

Les arrow functions offrent une syntaxe plus concise pour déclarer des fonctions.
...
Bien que les arrow functions soient la syntaxe moderne recommandée, elles ont quelques limitations :

```javascript
// ❌ Ne peuvent pas être utilisées comme constructeurs
const Person = (name) => {
  this.name = name; // Ne fonctionne pas
};
// const p = new Person('Alice'); // TypeError

// ✅ Utilisez une fonction classique pour les constructeurs
...
// ✅ Fonction classique si vous avez besoin d'arguments
function sum() {
...
<Callout type="info">
En pratique, ces limitations sont rarement un problème. **Les arrow functions restent le choix par défaut** pour la plupart des cas d'usage en JavaScript moderne.
</Callout>
...
- Les arrow functions sont la syntaxe moderne à privilégier
