...
<QuickCheck
  question="Quel est l'outil principal pour deboguer React ?"
  options={[
    "console.log() partout",
    "Deviner au hasard",
    "React DevTools (extension navigateur)",
    "Demander a l'IA sans regarder le code"
  ]}
  correctAnswer={2}
  explanation="React DevTools est l'extension navigateur dediee : elle permet d'inspecter les composants, voir les props/state en temps reel, profiler les performances et tracer les renders. console.log() aide ponctuellement mais ne remplace pas un outil dedie."
/>
...
<QuickCheck
  question="Que signifie l'erreur 'Cannot read property of undefined' ?"
  options={[
    "React est casse",
    "Vous essayez d'acceder a une propriete d'une valeur undefined",
    "Votre ordinateur a un probleme",
    "Il faut reinstaller Node.js"
  ]}
  correctAnswer={1}
  explanation="Cette erreur signifie que vous essayez d'acceder a une propriete (ex: user.name) alors que la variable (user) est undefined ou null. Solutions : verifier que les donnees existent avec optional chaining (user?.name), ajouter un if de garde, ou initialiser l'etat avec une valeur par defaut."
/>
...
<QuickCheck
  question="Quelle est la cause de l'erreur 'Too many re-renders' ?"
  options={[
    "Le composant a trop de props",
    "Un appel a setState directement dans le corps du composant (pas dans un handler)",
    "Le fichier est trop long",
    "React ne supporte pas autant de composants"
  ]}
  correctAnswer={1}
  explanation="Cette erreur arrive quand vous appelez setState directement dans le render (pas dans un handler ou useEffect), creant une boucle infinie : render -> setState -> re-render -> setState -> ... La solution est de placer setState dans un event handler ou dans useEffect."
/>
...
<QuickCheck
  question="Quelle est la meilleure strategie pour isoler un bug dans un composant React ?"
  options={[
    "Reecrire tout le composant depuis zero",
    "Commenter des parties du code jusqu'a ce que l'erreur disparaisse",
    "Ajouter plus de fonctionnalites et esperer que ca se corrige",
    "Ignorer le bug et continuer"
  ]}
  correctAnswer={1}
  explanation="Commenter progressivement des parties du code permet d'isoler exactement quelle section cause le probleme. C'est plus rapide que de tout reecrire et plus systematique que de deviner. Une fois le probleme isole, vous pouvez le corriger precisement."
/>
