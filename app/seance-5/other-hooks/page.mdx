...
<Callout type="info" title="Rappel : re-render">
...
<Callout type="warning" title="Difference importante">
**useState :**
...
<QuickCheck
  question="Quelle est la difference entre useRef et useState ?"
  options={[
    "Aucune difference",
    "useRef est plus rapide",
    "useRef ne declenche pas de re-render, useState oui",
    "useState est obsolete"
  ]}
  correctAnswer={2}
  explanation="La difference cle : useState declenche un re-render quand la valeur change, useRef ne declenche JAMAIS de re-render. Utilisez useState pour les donnees affichees dans l'UI, useRef pour les valeurs techniques (timers, references DOM, valeurs precedentes)."
/>
...
<QuickCheck
  question="Quand utiliser useMemo ?"
  options={[
    "Pour tous les calculs, toujours",
    "Pour creer des etats",
    "Pour memoriser le resultat d'un calcul couteux, seulement si necessaire",
    "Pour faire des appels API"
  ]}
  correctAnswer={2}
  explanation="useMemo memorise le resultat d'un calcul couteux pour eviter de le recalculer a chaque render. Utilisez-le seulement si le calcul est vraiment lourd (filtrage/tri de grandes listes). Ne l'utilisez pas partout par defaut : cela ajoute de la complexite sans benefice pour les calculs rapides."
/>
...
<QuickCheck
  question="A quoi sert useCallback ?"
  options={[
    "Creer des fonctions asynchrones",
    "Remplacer useState",
    "Memoriser une fonction pour eviter de la recreer a chaque render",
    "Faire des appels API"
  ]}
  correctAnswer={2}
  explanation="useCallback memorise une fonction pour qu'elle garde la meme reference entre les renders. C'est utile quand vous passez des callbacks a des composants enfants memorises avec React.memo, pour eviter des re-renders inutiles. Comme useMemo, utilisez-le seulement si necessaire."
/>

## Quand utiliser ces hooks ?

<Callout type="warning" title="N'optimisez pas trop tot">
...
<QuickCheck
  question="A quoi sert useRef avec un element DOM ?"
  options={[
    "A afficher l'element dans la console",
    "A acceder directement a l'element DOM (ex: focus, scroll, play)",
    "A supprimer l'element du DOM",
    "A remplacer getElementById"
  ]}
  correctAnswer={1}
  explanation="useRef cree une reference vers un element DOM, accessible via ref.current. Cela permet d'appeler des methodes DOM comme .focus(), .scrollIntoView(), .play() directement depuis React, sans passer par document.getElementById()."
/>
