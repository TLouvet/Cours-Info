...
Les custom hooks vous permettent de reutiliser la logique stateful entre differents composants.
...
<QuickCheck
  ...
  options={[
    "Un composant React special",
    "Une methode de classe",
    "Une fonction JavaScript qui utilise des hooks React",
    "Une librairie externe"
  ]}
  correctAnswer={2}
  explanation="Un custom hook est une fonction JavaScript (prefixee par 'use') qui utilise des hooks React (useState, useEffect, etc.) pour extraire et reutiliser de la logique stateful. Ce n'est pas un composant (pas de JSX retourne) ni une librairie externe."
/>
...
<Callout type="warning" title="Regles importantes">
...
<QuickCheck
  question="Quelle convention de nommage est obligatoire pour les custom hooks ?"
  options={[
    "Commencer par 'custom' (ex: customFetch)",
    "Commencer par 'hook' (ex: hookFetch)",
    "Toujours commencer par 'use' (ex: useFetch)",
    "Pas de convention specifique"
  ]}
  correctAnswer={2}
  explanation="Les hooks commencent TOUJOURS par 'use' (useFetch, useLocalStorage, useForm). Le prefixe 'use' permet a React d'appliquer les regles des hooks automatiquement (detection des appels conditionnels, etc.)."
/>
...
<QuickCheck
  question="Que retourne generalement un custom hook ?"
  options={[
    "Toujours du JSX",
    "Uniquement des strings",
    "Un objet ou tableau avec etat et fonctions",
    "Rien (void)"
  ]}
  correctAnswer={2}
  explanation="Les custom hooks retournent generalement un objet { data, loading, error } ou un tableau [value, setValue] comme useState. Ils ne retournent PAS de JSX (ca c'est pour les composants). Retourner un objet permet le renommage facile : const { data: users } = useFetch('/api/users')."
/>
...
<QuickCheck
  question="Pourquoi retourner un objet plutot qu'un tableau dans un custom hook ?"
  options={[
    "C'est obligatoire en React",
    "C'est plus rapide en terme de performance",
    "Cela permet le renommage facile lors de la destructuration",
    "Les tableaux ne fonctionnent pas avec les hooks"
  ]}
  correctAnswer={2}
  explanation="Un objet permet la destructuration nommee et le renommage facile : const { data: users } = useFetch('/api/users'). Avec un tableau, l'ordre est impose et le renommage est moins lisible. Les tableaux fonctionnent (useState en retourne un), mais les objets sont plus flexibles pour les hooks avec plusieurs valeurs."
/>
